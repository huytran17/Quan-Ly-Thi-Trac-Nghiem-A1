...
if (isset($_GET['editprofile'])) {
    if ($thisinh = $ts->getById($_GET['editprofile'])->fetch_assoc()) {
        extract($thisinh);
        $avatar_path = $avatar_photo_path;
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if (!$hasError) {
        $affected_rows = $ts->update($editprofile, [
            'mathisinh' => $mathisinh,
            'hoten' => $hoten,
            'ngaysinh' => $ngaysinh,
            'gioitinh' => $gioitinh,
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-6">
                ...
                <input type="date" name="ngaydangkythi" class="form-control" value="<?php echo isset($ngaydangkythi) ? $ts->ymdFormat($ngaydangkythi) : '' ?>" required />
                <span class="error"><?php echo $errors['ngaydangkythi'] ?></span>
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <img src="<?php echo isset($avatar_photo_path) ? $avatar_photo_path : '' ?>" alt="<?php echo isset($hoten) ? $hoten : '' ?>" width="200" height="200">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
