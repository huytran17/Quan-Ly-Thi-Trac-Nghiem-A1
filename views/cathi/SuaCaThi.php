...
if (isset($_GET['editshift'])) {
    if ($cathiTS = $ct->getByIdWithThiSinh($_GET['editshift'])->fetch_assoc()) extract($cathiTS);
    if ($cathiPT = $ct->getByIdWithPhongThi($_GET['editshift'])->fetch_assoc()) extract($cathiPT);
}
...
if (isset($_POST['submit'])) {
    ...
    if (!$hasError) {
        $affected_rows = $ct->update($editshift, [
            ...
            <div class="form-group">
                ...
                <select name="phongthi_id" class="form-control">
                    <?php while ($obj = $allPT->fetch_object()) { ?>
                        <option value="<?php echo $obj->id ?>" <?php echo $obj->id == $phongthi_id ? 'selected' : ''; ?>>
